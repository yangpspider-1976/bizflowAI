...
export default function NewClientPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { data, error } = await supabase.from('clients').insert({
      ...form,
      target_customers: form.target_customers ? form.target_customers.split(',').map(s => s.trim()) : [],
    }).select('id').single()
    if (error) {
    ...
    <div className="p-8 max-w-3xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex gap-3">
          ...
          <Link href="/clients" className="px-6 py-2 rounded-md font-medium border border-gray-300 text-gray-700 hover:bg-gray-50 transition-colors">
            Cancel
          </Link>
        </div>
      </form>
